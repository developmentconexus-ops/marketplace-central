...
function assertRatifiedAuthority(text) {
  ...
  requireText(text, 'D7R_IDEMPOTENCY_FINGERPRINT:REQUEST_IFMATCH_OUTCOME', 'D7-R idempotency fingerprint');
  ...
  requireText(text, 'D7R_REPLAY_ORDER:IDEMPOTENCY_BEFORE_IF_MATCH', 'D7-R replay ordering');
...
function expectFailure(name, mutate) {
  let failed = false;
  try { assertRatifiedAuthority(mutate(doc)); } catch { failed = true; }
  assert(failed, `D7-R negative control unexpectedly passed: ${name}`);
  negativeControls++;
}

assertRatifiedAuthority(doc);
...
expectFailure('replay-before-If-Match can be removed', (text) =>
  text.replace('D7R_REPLAY_ORDER:IDEMPOTENCY_BEFORE_IF_MATCH', 'D7R_REPLAY_ORDER:IF_MATCH_BEFORE_IDEMPOTENCY'));
...
console.log('authorization_request_d7r_replay_order=IDEMPOTENCY_BEFORE_IF_MATCH');
