...
const sourceFiles = sourceNames.map((name) => join(contractDir, name));
const sourceTreeText = sourceFiles.map((path) => readFileSync(path, 'utf8')).join('\n');
...
function resolveRef(document, value, seen = new Set()) {
  ...
  return resolveRef(document, current, nextSeen);
}
...
function operations(document) {
  ...
  for (const [path, pathItemRaw] of Object.entries(document.paths ?? {})) {
    const pathItem = resolveRef(document, pathItemRaw);
...
function requestSchema(document, operation, mediaType = 'application/json') {
  ...
  const raw = body?.content?.[mediaType]?.schema;
  return raw ? resolveRef(document, raw) : undefined;
}
...
function component(document, kind, name) {
  ...
  const entries = Object.entries(document.components?.[kind] ?? {});
  const suffix = entries.find(([candidate]) => candidate === name || candidate.endsWith(`_${name}`) || candidate.endsWith(name));
  assert(suffix, `bundled component not found: ${kind}.${name}`);
  return resolveRef(document, suffix[1]);
}
...
function parseW4(text) {
  ...
  for (const line of text.slice(start, end).split(/\r?\n/)) {
    ...
    const principalKinds = qualifiedPhysical ? ['H', 'S'] : [...principalCell.matchAll(/\b([HAS])\b/g)].map((m) => m[1]);
    assert(operationClass && permission && principalKinds.length, `unparseable W4 row: ${line}`);
    rows.push({ operationId, operationClass, permission, principalKinds: normalize(principalKinds), qualifiedPhysical });
  }
...
function parseIdempotencyOps(text) {
  ...
  assert(result.length === 14, `mandatory Idempotency-Key count changed: ${result.length}`);
  return normalize(result);
}

function parseSafetySweepOps(text) {
  const start = text.indexOf('# 7. Whole-Matrix complete C-operation safety sweep');
  ...
  return normalize([...text.slice(start, end).matchAll(/^\|\s*`([^`]+)`\s*\|/gm)].map((m) => m[1]).filter((id) => id !== 'Operation'));
}
...
const expectedIdempotency = parseIdempotencyOps(admissionText);
const expectedCOperations = normalize(w4.filter((row) => row.operationClass === 'C').map((row) => row.operationId));
const admittedCOperations = parseSafetySweepOps(admissionText);
assert(expectedPermissions.length === 29, `ordinary Permission count changed: ${expectedPermissions.length}`);
sameSet(admittedCOperations, expectedCOperations, 'W4/admission C-operation coverage');
...
function validate(document) {
  ...
  for (const entry of all) {
    ...
    assert(typeof op['x-mpc-semantic-owner'] === 'string' && op['x-mpc-semantic-owner'].length > 0, `${op.operationId} has no semantic owner`);
  }

  const permissionSchema = component(document, 'schemas', 'Permission');
  sameSet(permissionSchema.enum ?? [], expectedPermissions, 'Permission enum');
  ...
  const forbiddenQuery = new Set(['page', 'offset', 'sort', 'total', 'total_count']);
  for (const entry of listSearch) {
    const queryNames = parameters(document, entry).filter((parameter) => parameter.in === 'query').map((parameter) => parameter.name);
    assert(queryNames.includes('limit') && queryNames.includes('cursor'), `${entry.operation.operationId} lacks limit/cursor`);
    assert(!queryNames.some((name) => forbiddenQuery.has(name)), `${entry.operation.operationId} contains offset/sort/total grammar`);
  ...
  assert(limit.schema?.type === 'integer' && Number.isInteger(limit.schema.default) && Number.isInteger(limit.schema.maximum), 'Limit lacks explicit integer default/maximum');
  assert(limit.schema.minimum === 1 && limit.schema.default > 0 && limit.schema.maximum >= limit.schema.default, 'Limit bounds are incoherent');
  ...
  const methodNotAllowed = component(document, 'responses', 'MethodNotAllowed');
  assert(methodNotAllowed.headers?.Allow, '405 MethodNotAllowed lacks Allow');
  ...
  for (const entry of all) {
    if (parameters(document, entry).some((parameter) => parameter.in === 'header' && parameter.name.toLowerCase() === 'idempotency-key')) idempotencyOps.push(entry.operation.operationId);
  }
  ...
  const authTarget = component(document, 'schemas', 'AuthorizationTarget');
  for (const branch of authTarget.oneOf ?? []) {
  ...
  for (const id of durableCreates) {
    const created = response(document, byId(all, id).operation, '201');
    assert(created?.headers?.Location, `${id} must return 201 + Location`);
  }
  ...
  const workOrigin = component(document, 'schemas', 'WorkOrigin');
  const workOriginKinds = (workOrigin.oneOf ?? []).map((branch) => {
    const schema = resolveRef(document, branch);
...
function expectFailure(name, candidateFactory) {
  let failed = false;
  try { validate(candidateFactory()); } catch { failed = true; }
  assert(failed, `negative control unexpectedly passed: ${name}`);
}

function negativeSemanticControls(document) {
  expectFailure('missing operation', () => {
    const candidate = clone(document);
    delete candidate.paths['/organizations/{organization_id}/work/{work_id}:escalate'];
    return candidate;
  });
  expectFailure('wrong W4 permission', () => {
    const candidate = clone(document);
    byId(operations(candidate), 'CreatePriceIntent').operation['x-mpc-required-permission'] = 'listing.manage';
    return candidate;
  });
  expectFailure('technical path leakage', () => {
    const candidate = clone(document);
    candidate.paths['/organizations/{organization_id}/webhooks'] = clone(candidate.paths['/organizations/{organization_id}/work']);
    return candidate;
  });
  expectFailure('fourth principal kind', () => {
    const candidate = clone(document);
    byId(operations(candidate), 'GetCurrentAccessContext').operation['x-mpc-principal-kinds'].push('X');
    return candidate;
  });
  expectFailure('missing idempotency carrier', () => {
    const candidate = clone(document);
    const entry = byId(operations(candidate), 'CreateInventorySource');
    entry.operation.parameters = (entry.operation.parameters ?? []).filter((raw) => resolveRef(candidate, raw)?.name !== 'Idempotency-Key');
    return candidate;
  });
  expectFailure('wrong Product Problem origin', () => {
    const candidate = clone(document);
    const schema = component(candidate, 'schemas', 'AccessDeniedProblem');
    schema.properties.type.const = 'https://example.invalid/problem';
    return candidate;
  });
}
...
function redoclyProof() {
  ...
  return { document: JSON.parse(readFileSync(bundleA, 'utf8')), bundleA };
}

function typescriptProof() {
  const a = join(temp, 'product-a.d.ts');
  const b = join(temp, 'product-b.d.ts');
  run(npx, ['--yes', 'openapi-typescript@7.13.0', entrypoint, '-o', a, '--redocly', redoclyConfig]);
  run(npx, ['--yes', 'openapi-typescript@7.13.0', entrypoint, '-o', b, '--redocly', redoclyConfig]);
  assert(sha256(a) === sha256(b), 'TypeScript projection is not deterministic');
  ...
  assert(createHash('sha256').update(`${generated}\n// deliberate drift\n`).digest('hex') !== sha256(a), 'TypeScript drift negative control failed');
}
...
function goProof(bundlePath) {
  ...
  run(go, ['run', 'github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@v2.8.0', '-config', 'oapi-codegen.yaml', bundlePath], { cwd: dir });
  const generated = readFileSync(join(dir, 'product.gen.go'), 'utf8');
  ...
  run(go, ['test', './...'], { cwd: dir });
  const currentVersion = run(go, ['version'], { cwd: dir }).stdout.trim();
...
function legacyRuntimeProof() {
  ...
  const forbiddenPrefixes = ['apps/', 'cmd/', 'internal/', 'server/', 'backend/', 'frontend/'];
  for (const prefix of forbiddenPrefixes) assert(!tracked.some((path) => path.startsWith(prefix)), `active legacy/runtime population found under ${prefix}`);
  const openapiAuthorities = tracked.filter((path) => /(^|\/)openapi[^/]*\.ya?ml$/i.test(path));
  sameSet(openapiAuthorities, ['contracts/api/product/openapi.yaml'], 'tracked OpenAPI entrypoint authority');
}
...
try {
  ...
  sourceTreeProof();
  legacyRuntimeProof();
  const { document, bundleA } = redoclyProof();
