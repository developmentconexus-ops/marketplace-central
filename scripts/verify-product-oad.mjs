...
sameSet(parseSafetySweepOps(admissionText), expectedCOperations, 'W4/admission C-operation coverage');
...
function goProof(bundlePath) {
  ...
  const config = join(dir, 'oapi-codegen.yaml');
  writeFileSync(config, ['package: productapi', 'output: product.gen.go', 'generate:', '  models: true', '  std-http-server: true', '  strict-server: true', 'output-options:', '  skip-prune: true', ''].join('\n'));
  ...
  assert(run(go, ['list', '-m', '-f', '{{.Version}}', 'github.com/oapi-codegen/runtime'], { cwd: dir }).stdout.trim() === 'v1.7.0', 'unexpected oapi-codegen runtime');
