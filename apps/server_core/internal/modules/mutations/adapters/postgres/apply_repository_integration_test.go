...
func TestApplyRepositorySnapshotClaimChunkAndImmutableOutcomes(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `SELECT state FROM mutation_items WHERE tenant_id=$1 AND protocol_id=$2 AND item_id=$3`, tenant, created.ProtocolID, chunk[0].ItemID).Scan(&visibleBeforeCommit); err != nil || visibleBeforeCommit != domain.ItemStateApplied {
		t.Fatalf("outcome before claim commit=%q err=%v", visibleBeforeCommit, err)
	}
