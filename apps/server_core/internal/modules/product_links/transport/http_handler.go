...
// LinkWorkflowResolver additionally carries the S4 undo surface
// (UndoResolution/UndoBatch). It is widened rather than split into a new
// interface because *application.ResolutionService — already injected as
// workflowResolver in NewHandler/NewHandlerWithBatchPreview — implements
// both the existing methods and these new ones: no new constructor
// parameter, no new root.go wiring required for undo.
type LinkWorkflowResolver interface {
	...
	UndoResolution(ctx context.Context, input application.UndoResolutionInput) (domain.ProductLinkResolutionResult, error)
	UndoBatch(ctx context.Context, batchID string) (application.UndoBatchResult, error)
}
...
func (h Handler) Register(mux httpx.RouteRegistrar) {
	...
	mux.HandleFunc("/product-links/link-resolutions/{id}/undo", h.handleUndoResolution)
...
func mapProductLinksError(err error) (int, string, string) {
	...
	// S4 undo ordering conflicts — byte-exact codes per IC-01/F-01 (§3):
	// SUPERSEDED and ALREADY_UNDONE are both 409, never a generic 400/404.
	if errors.Is(err, application.ErrProductLinkSuperseded) || errors.Is(err, application.ErrProductLinkAlreadyUndone) {
		return http.StatusConflict, msg, msg
	}
...
func (h Handler) handleUndoResolution(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
	if r.Method != http.MethodPost {
		w.Header().Set("Allow", http.MethodPost)
	...
	// A body is optional for undo (reason/actor default to zero values); a
	// malformed non-empty body is still rejected as 400.
	if r.Body != nil {
	...
	result, err := h.workflowResolver.UndoResolution(r.Context(), application.UndoResolutionInput{
		AuditID: auditID,
		Actor:   req.Actor,
		Reason:  req.Reason,
	})
...
func (h Handler) handleUndoBatch(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
	if r.Method != http.MethodPost {
		w.Header().Set("Allow", http.MethodPost)
	...
	result, err := h.workflowResolver.UndoBatch(r.Context(), batchID)
	if err != nil {
		status, code, message := mapProductLinksError(err)
