...
func ParseListingQuery(values url.Values) (ListingQuery, error) {
	...
	for key := range values {
		if strings.HasPrefix(key, "filter.") {
			...
			if !domain.IsFilterKey(name) {
				return ListingQuery{}, invalidFilter(name)
			}
		}
	}
	...
	for _, key := range domain.FilterKeys {
		value, err := filterScalar(values, key)
		if err != nil {
			return ListingQuery{}, err
		}
		if value != "" {
			if err := domain.SetFilterValue(&result.Filter, key, value); err != nil {
				return ListingQuery{}, invalidFilter(key)
			}
		}
	}
	return result, nil
