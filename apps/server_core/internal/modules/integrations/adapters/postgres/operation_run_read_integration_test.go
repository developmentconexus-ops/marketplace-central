...
func TestListRunsTenantScopedNewestFirstWithCursor(t *testing.T) {
	ctx, repo, pool, tenant, other, installation, otherInstallation := runReadHarness(t, "cursor")
	...
	seedReadRun(t, pool, other, otherInstallation, "leak", "listing_read", domain.OperationRunStatusSucceeded, base.Add(time.Hour), &base)
	...
	scoped, err := repo.ListRuns(ctx, ports.RunListQuery{InstallationID: otherInstallation})
	if err != nil || len(scoped.Items) != 0 || scoped.NextCursor != nil {
		t.Fatalf("other-tenant installation leaked: page=%+v err=%v", scoped, err)
	}
}

func TestListRunsExcludesStartedBeforeNinetyDays(t *testing.T) {
	ctx, repo, pool, tenant, _, installation, _ := runReadHarness(t, "window")
	now := time.Now().UTC()
...
func TestListRunsFiltersStatusAndModule(t *testing.T) {
	ctx, repo, pool, tenant, _, installation, _ := runReadHarness(t, "filters")
	now := time.Now().UTC()
...
func TestRunningRunPreservesNullFinishedAt(t *testing.T) {
	ctx, repo, pool, tenant, _, installation, _ := runReadHarness(t, "null-finished")
	now := time.Now().UTC()
...
func runReadHarness(t *testing.T, name string) (context.Context, *integrationspostgres.OperationRunRepository, *pgxpool.Pool, string, string, string, string) {
	t.Helper()
	ctx := context.Background()
	pool, _ := testpostgres.OpenPool(t, "operation_run_read_"+name)
	token := fmt.Sprintf("%d", time.Now().UnixNano())
	tenant, other := "run-a-"+token, "run-b-"+token
	installation, otherInstallation := "run-inst-a-"+token, "run-inst-b-"+token
