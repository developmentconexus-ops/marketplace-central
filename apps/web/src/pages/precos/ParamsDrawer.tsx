        ...
        <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-[18px] py-4">
              ...
              {REGIMES.map((rg) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={on}
                    onClick={() =>
                      setDraft((d) => ({ ...d, regime: rg.key, aliquota_pct: rg.aliquota }))
                    }
          ...
          <section className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <PctInput
                aria="Limiar amarela"
                value={draft.limiar_amarelo_pct}
          ...
          <section data-testid="params-modalidades-ml" className="flex flex-col gap-2">
            ...
            <ModalidadeRow
              ...
              active={modalidade === "classico"}
              tarifa={comissaoTarifa}
            />
            <ModalidadeRow
              label="Comissão Premium"
              active={modalidade === "premium"}
              tarifa={comissaoTarifa}
            />
            <div className="flex items-center gap-2.5">
              ...
              <span className="flex w-[78px] flex-none items-center gap-1 rounded-control border border-border px-2.5 py-[5px]">
                ...
                <input
                  ...
                  placeholder="—"
                  onChange={(e) =>
                    set("tarifa_full", e.target.value.trim() === "" ? null : e.target.value)
                  }
          ...
          <section className="flex flex-col gap-2">
            ...
            <p data-testid="params-difal-context" className="text-[11.5px] leading-snug text-muted">
              ...
              {draft.difal_destino_uf ? (
                <>
                  Destino atual: <b className="text-ink">{draft.difal_destino_uf}</b> (do CEP da
                  análise).
                </>
              ) : (
            ...
            {draft.difal_enabled ? (
              <label className="flex items-center gap-2.5 text-muted">
                ...
                <select
                  ...
                  onChange={(e) =>
                    set("difal_destino_uf", e.target.value === "" ? null : e.target.value)
                  }
...
function SectionLabel({
  ...
  return (
    <div className={`text-[10.5px] font-semibold tracking-[0.08em] text-faint ${className}`}>
      {children}
    </div>
  );
}
    ...
    <div className="flex items-center gap-2.5">
      ...
      {show ? (
        <span className="font-mono font-semibold text-ink">
          {formatPercent(tarifa!.valor) ?? `${tarifa!.valor}%`}
        </span>
      ) : (
        <UnknownValue />
      )}
      <span className="text-[10.5px] text-faint">
        {fonte ? `${fonte} · vem do ML` : "vem do ML"}
      </span>
    </div>
  );
}
