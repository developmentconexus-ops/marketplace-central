...
function QualityChip({ flag }: { flag: QualityFlag }) {
  const className =
    flag === "complete" ? "bg-accent-soft text-accent-ink" : "bg-warn-soft text-warn";
  return (
    <span
...
export function ProdutoHeader({ productId }: ProdutoHeaderProps) {
  ...
  return (
    <header
      aria-labelledby="produto-header-title"
