...
const { getMutation, listMutationItems, retryMutationFailures, invalidateAfterMutation } =
  vi.hoisted(() => ({
    getMutation: vi.fn(),
...
vi.mock("../../app/ClientContext", () => ({
  useClient: () => ({ getMutation, listMutationItems, retryMutationFailures }),
}));
...
function mount(
  initialEntry = "/protocolos/MP-000042",
...
describe("ProtocoloPage", () => {
  ...
  it("retoma polling no acesso direto, para no terminal e invalida uma vez", async () => {
    getMutation
      ...
      .mockResolvedValue({ ...applying, state: "applied", finished_at: "2026-07-17T12:00:03Z" });
    mount();
    ...
    expect(
      screen.queryByRole("button", { name: "Repetir itens com falha" }),
  ...
  it("mostra cabeçalho, valores desconhecidos, falha e detalhe técnico sob demanda", async () => {
    ...
    expect(screen.getByRole("link", { name: "MP-000001" })).toHaveAttribute(
      "href",
      "/protocolos/MP-000001",
    );
  ...
  it("pagina os itens pelo cursor retornado", async () => {
    ...
    listMutationItems
      .mockResolvedValueOnce({
        items: [{ ...failedItem, failure: null, state: "applied" }],
        next_cursor: "cursor-2",
        page_size: 1,
      })
      .mockResolvedValueOnce({
        items: [{ ...failedItem, item_id: "item-2", listing_id: "MLB-2" }],
        next_cursor: null,
        page_size: 1,
      });
    mount();
    ...
    expect(listMutationItems).toHaveBeenLastCalledWith("MP-000042", {
      limit: 50,
      cursor: "cursor-2",
    });
  });
  ...
  it("repete falhas uma vez, preserva a instalação e abre o novo protocolo", async () => {
    ...
    const retried = {
      ...failed,
      protocol_id: "MP-000043",
      retried_from: "MP-000042",
      state: "applied" as const,
    };
    let resolveRetry!: (protocol: MutationProtocol) => void;
    retryMutationFailures.mockReturnValue(
      new Promise<MutationProtocol>((resolve) => {
    ...
    expect(screen.getByRole("link", { name: "MP-000042" })).toHaveAttribute(
      "href",
      "/protocolos/MP-000042",
    );
    expect(screen.getByTestId("location")).toHaveTextContent(
      "/protocolos/MP-000043?installation=inst_test&tab=failed",
    );
  });
