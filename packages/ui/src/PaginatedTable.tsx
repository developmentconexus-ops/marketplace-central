  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12 text-muted text-sm">
        <div className="animate-spin rounded-full h-5 w-5 border-2 border-border border-t-accent mr-3" />
        Loading...
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={handlePrev}
            disabled={safePage === 1}
            className="inline-flex items-center gap-1 px-2 py-1.5 text-xs font-medium rounded-control border border-border text-muted hover:bg-surface-2 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
          >
            <ChevronLeft className="w-3.5 h-3.5" />
            Prev
          </button>
          <span className="text-xs text-muted font-medium min-w-[80px] text-center">
