...
describe("sdk runtime", () => {
  ...
  it("keeps canonical nullable fields required across OpenAPI and SDK", () => {
    ...
    const productSchema = openapi.slice(
      openapi.indexOf("    CanonicalCatalogProduct:"),
      openapi.indexOf("    CatalogProduct:"),
    );
    const catalogFactSchema = openapi.slice(
      openapi.indexOf("    CatalogProductFact:"),
      openapi.indexOf("    CatalogQuantityFact:"),
    );
    const canonicalSdk = sdk.slice(
      sdk.indexOf("export interface CanonicalCatalogProduct"),
      sdk.indexOf("export interface CatalogProductFact"),
    );
    const catalogFactSdk = sdk.slice(
      sdk.indexOf("export interface CatalogProductFact"),
      sdk.indexOf("export interface CatalogProductFactPage"),
    );
    ...
    expect(productSchema).toContain(
      "required: [internal_product_id, name, ean, manufacturer_reference, seller_sku, brand_name, product_group_name, cost_amount, price_amount, stock_quantity, ncm, quality_flags]",
    );
    for (const field of [
      "ean",
    ...
    expect(catalogFactSchema).toContain(
      "required: [internal_product_id, reference, description, ean, active, sellable_stock, current_price, cost, manufacturer_reference, brand_name, ncm, quality_flags]",
    );
    expect(catalogFactSchema).toMatch(
    ...
    const qualityFlagSchema = openapi.slice(
      openapi.indexOf("    QualityFlag:"),
      openapi.indexOf("    CanonicalCatalogProduct:"),
    );
    const qualityFlagSdk = sdk.slice(
      sdk.indexOf("export type QualityFlag"),
      sdk.indexOf("export interface CanonicalCatalogProduct"),
    );
    for (const flag of [
      "complete",
    ...
    const wireFact: CatalogProductFact = {
      ...catalogFact,
      quality_flags: ["complete", "ean_collision"],
    };
    const wireCanonical: CanonicalCatalogProduct = {
      ...
      cost_amount: {
        ...
        quality: "unknown",
        observed_at: null,
        quality_reason: "cost unavailable",
      },
      price_amount: {
        source: "oracle",
        value: null,
        quality: "unknown",
        observed_at: null,
        quality_reason: "price unavailable",
      },
      stock_quantity: {
        source: "oracle",
        value: null,
        quality: "unknown",
        observed_at: null,
        quality_reason: "stock unavailable",
      },
    };
  ...
  it("lists catalog facts with the IC-01 page envelope", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/catalog/products?cursor=MTIz&limit=25",
    );
    expect(String(requests[1].input)).toBe(
      "http://localhost:8080/catalog/products/search?q=PARAFUSO&limit=50",
    );
  ...
  it("reads listing pages, grouped pages, details, and summaries with server filter syntax", async () => {
    ...
    const grouped = await client.listListingsByProduct({
      installation_id: "install/1",
      link_state: "resolved",
    });
    ...
    expect(String(requests[2].input)).toBe(
      "http://localhost:8080/listings/install%2F1~provider%2F1~-",
    );
    expect(String(requests[3].input)).toBe(
      "http://localhost:8080/listings/summary?installation_id=install%2F1",
    );
  ...
  it("rejects non-positive product-link CODPROD before transport", () => {
    ...
    for (const internal_product_id of [-1, 0]) {
      expect(() => client.manualResolveProductLink({ ...base, internal_product_id })).toThrow(
        "invalid_identity",
      );
    }
  ...
  it("starts integration authorize flow with auth_url and expires_in", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/auth/authorize",
    );
  ...
  it("starts integration reauthorization flow with auth_url and expires_in", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/reauth/authorize",
    );
    expect(requests[0].init?.method).toBe("POST");
  ...
  it("submits integration credentials as json and parses auth status", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/auth/credentials",
    );
    expect(requests[0].init?.method).toBe("POST");
  ...
  it("disconnects integration installation and parses auth status", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/disconnect",
    );
    expect(requests[0].init?.method).toBe("POST");
  ...
  it("gets integration auth status", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/auth/status",
    );
    expect(requests[0].init?.method).toBe("GET");
    expect(result.status).toBe("connected");
  ...
  it("probes integration account", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/probes/account",
    );
    expect(requests[0].init?.method).toBe("POST");
  ...
  it("reads integration listings probe", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/probes/listings?limit=3",
    );
    expect(requests[0].init?.method).toBe("GET");
  ...
  it("reads integration orders probe", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/probes/orders?limit=1",
    );
    expect(requests[0].init?.method).toBe("GET");
  ...
  it("reads integration fee quote probe", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/integrations/installations/inst-1/probes/fee-quote?listing_type_id=gold_special&price=100&currency_id=BRL",
    );
    expect(requests[0].init?.method).toBe("GET");
  ...
  it("imports product link listing snapshots", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/product-links/listing-snapshots/imports",
    );
    expect(requests[0].init?.method).toBe("POST");
    expect(requests[0].init?.headers).toEqual({ "Content-Type": "application/json" });
  ...
  it("refreshes listings and preserves the active operation on conflict", async () => {
          ...
          return new Response(
            JSON.stringify({
              error: {
                ...
                message: "busy",
                details: { operation_run_id: "op_active" },
              },
            }),
            { status: 409, headers: { "Content-Type": "application/json" } },
          );
        }
        return new Response(JSON.stringify({ operation_run_id: "op_new" }), {
          status: 202,
          headers: { "Content-Type": "application/json" },
        });
      },
    ...
    await expect(client.refreshListings({ installation_id: "inst_test" })).resolves.toEqual({
      operation_run_id: "op_new",
    });
    ...
    conflict = true;
    await expect(client.refreshListings({ installation_id: "inst_test" })).rejects.toMatchObject({
      status: 409,
      error: { code: "refresh_in_progress", details: { operation_run_id: "op_active" } },
    });
  });
  ...
  it("generates product link candidates as json", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/product-links/link-candidates/generations",
    );
    expect(requests[0].init?.method).toBe("POST");
    expect(requests[0].init?.headers).toEqual({ "Content-Type": "application/json" });
  ...
  it("approves product link candidates as json", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/product-links/link-resolutions/approve-candidate",
    );
    expect(requests[0].init?.method).toBe("POST");
    expect(requests[0].init?.body).toBe(
      JSON.stringify({
  ...
  it("applies inventory manual stock action as json", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/inventory/stock-actions/manual-apply",
    );
    expect(requests[0].init?.method).toBe("POST");
    expect(requests[0].init?.body).toBe(
      JSON.stringify({
  ...
  it("lists marketplace orders with normalized snapshots", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/orders?installation_id=inst-1&limit=5",
    );
    expect(requests[0].init?.method).toBe("GET");
  ...
  it("imports profitability margin inputs as json", async () => {
    ...
    const result = await client.importProfitabilityMarginInputs({
      installation_id: "inst-1",
      limit: 1,
    });

    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/profitability/margin-inputs/import",
    );
    expect(requests[0].init?.method).toBe("POST");
    expect(result.imported_count).toBe(2);
  ...
  it("lists profitability margin inputs", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/profitability/margin-inputs?installation_id=inst-1&limit=5",
    );
  ...
  it("creates profitability manual adjustment as json", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/profitability/manual-adjustments",
    );
    expect(requests[0].init?.method).toBe("POST");
  ...
  it("calculates profitability profit snapshots as json", async () => {
    ...
    const result = await client.calculateProfitabilityProfitSnapshots({
      installation_id: "inst-1",
      limit: 1,
    });

    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/profitability/profit-snapshots/calculate",
    );
    expect(requests[0].init?.method).toBe("POST");
  ...
  it("lists profitability profit snapshots", async () => {
    ...
    expect(String(requests[0].input)).toBe(
      "http://localhost:8080/profitability/profit-snapshots?installation_id=inst-1&limit=5",
    );
  ...
  it("listMutations gets an encoded filtered protocol page", async () => {
    ...
    const client = createMarketplaceCentralClient({
      ...
      fetchImpl: async (input, init) => {
        requests.push({ input, init });
        return new Response(
          JSON.stringify({ items: [{ protocol_id: "MP-1" }], next_cursor: null, page_size: 1 }),
          { status: 200 },
        );
      },
    });
  ...
  it("listMutationItems gets an encoded filtered item page", async () => {
    ...
    const client = createMarketplaceCentralClient({
      ...
      fetchImpl: async (input, init) => {
        requests.push({ input, init });
        return new Response(
          JSON.stringify({ items: [{ seq: 1 }], next_cursor: "next", page_size: 1 }),
          { status: 200 },
        );
      },
    });
  ...
  it("reads dashboard summary and sync runs with encoded query values and nullable fields", async () => {
    ...
    const client = createMarketplaceCentralClient({
      ...
      fetchImpl: async (input, init) => {
        requests.push({ input, init });
        return new Response(
          JSON.stringify(String(input).includes("/dashboard/") ? summary : runs),
          {
            status: 200,
            headers: { "Content-Type": "application/json" },
          },
  ...
  it.each([
    ...
    const client = createMarketplaceCentralClient({
      ...
      fetchImpl: async () =>
        new Response(
          JSON.stringify({ error: { code, message: "category attributes unavailable" } }),
          { status },
        ),
    });
  ...
  it("lists market observations and references with encoded ordered ids", async () => {
    ...
    const client = createMarketplaceCentralClient({
      ...
      fetchImpl: async (input, init) => {
        requests.push({ input, init });
        return new Response(JSON.stringify({ items: [], as_of: "2026-07-16T12:00:00Z" }), {
          status: 200,
        });
      },
    });
  ...
  it("drives the IC-04 pricing calculator endpoints (profile, difal, scenarios, decompose, solve)", async () => {
    ...
    const overrideResp: PricingDifalOverrideResponse = {
      ...
      rate: {
        ...difalList.items[0],
        efetivo_pct: "7.00",
        override: { interna_pct: "19.00", updated_at: "2026-07-18T00:00:00Z", actor: "op" },
      },
    };
    const scenario: PricingScenario = {
      id: "sc1",
      name: "Cenário",
      payload: { preco: "100.00" },
    ...
    const client = createMarketplaceCentralClient({
      ...
      fetchImpl: async (input, init) => {
        ...
        else if (path.endsWith("/pricing/scenarios"))
          body = method === "POST" ? scenario : { items: [scenario] };
        else if (path.endsWith("/pricing/decompose")) body = decompose;
        else if (path.endsWith("/pricing/solve")) body = solve;
        if (method === "DELETE") return new Response(null, { status: 204 });
        return new Response(JSON.stringify(body), {
          status: method === "POST" && path.endsWith("/scenarios") ? 201 : 200,
        });
      },
    });
    ...
    const putDifal = await client.putPricingDifalOverride("SP", {
      interna_pct: "19.00",
      actor: "op",
    });
    const listScenarios = await client.listPricingScenarios();
    const createScenario = await client.createPricingScenario({
      id: "sc1",
      name: "Cenário",
      payload: { preco: "100.00" },
    });
    await client.deletePricingScenario("sc1");
    const gotDecompose = await client.pricingDecompose({
      preco: "100.00",
      comissao_pct: "12",
      modalidade: "classico",
      product_id: 90001,
    });
    const gotSolve = await client.pricingSolveTarget({
      margem_alvo_pct: "95",
      comissao_pct: "16",
      modalidade: "classico",
      custo: "10.00",
  ...
  it("passes a null pricing decompose imposto through untouched (nullable field parity)", async () => {
    ...
    const got = await client.pricingDecompose({
      preco: "100.00",
      comissao_pct: "12",
      modalidade: "classico",
    });
