...
**Browser-drive seam — the hub BORROWS the file, it does not re-point the stack (ratified
2026-07-28, CHIP-ANCHORS-3 F-5):** a chip may not own the stack (§2 L2), so a screen criterion is
discharged by the HUB driving the CHIP's build. Re-pointing compose at the chip's worktree looks
like the way and is not: `docker compose` resolves each service's `env_file:` against the project
directory, a worktree has no `.env`, and `--env-file` does not override a service-level `env_file:`.
The workaround that suggests itself — creating a `.env` in the chip's worktree — is **forbidden**:
it would place live provider credentials inside a chip session's reachable tree. `-p <project>` is
still required for anything that does re-point, since compose derives the project name from the
directory and would otherwise fork a parallel stack on fresh volumes.

The available mechanism is narrower and cleaner: the hub does
`git checkout <chip-branch> -- <exact files>` into its OWN checkout, drives the running stack,
captures the rendered text, and reverts with `git checkout HEAD -- <same files>`, verifying
`git status --porcelain` before and after. The hub returns the captured text VERBATIM to the chip,
so the drive is the hub's while the reading stays the chip's — the chip is not certifying its own
screen, and the hub is not paraphrasing it. Borrow only files inside the grant already issued.
...
3. the executing seat's RAW lane outputs (ladder, must-fail, fixtures) — measurements, not prose
   about measurements;
4. **hub-MEASURED observables the seat cannot reproduce**, when any criterion asserts a fact about
   running reality — a wire dump, a rendered screen, a query result, captured by the HUB on a live
   stack and committed as a gate artifact. Verbatim capture only: no interpretation of the capture
   travels as input, and the seat is free to contradict it by naming a row and a field.

Item 4 is a carve-out, and it exists because the exclusion above guarantees a specific failure. The
chip cannot boot a server (§2 L2) and the seat has no browser, so a criterion about a SCREEN is
decided by whoever reasoned about the generator — and reasoning about a generator decides
producibility, never reachability (§11, PRODUCIBLE vs REACHABLE). Withhold the measurement and both
seats infer reachability from the same source the chip inferred it from, and reach the same wrong
conclusion BY CONSTRUCTION. That is not a risk, it is the mechanism: it ran for seven rounds on
CHIP-VINC-NEUTRO with two independent gates and neither could see it. The distinction that keeps
this from re-opening the pack: item 4 is the hub MEASURING reality, items excluded are the chip
ARGUING about it. A capture a seat can contradict is input; prose it can only agree or disagree
with is not.
...
```
2026-08-09 · §2 · ratified · guards lane added to the gate (issue #3: inventory at contracts/gate/guards.json, every registered guard held to an EXECUTED failing fixture; targeted go-test proof for Go detectors, delegation-to-selftest verified for pwsh fixtures) · scripts/arch-gate.sh DELETED, measured retirement: zero executable references at 60f4d457 (package.json, .github/workflows/*, Makefile, scripts/gate.ps1, scripts/harness.ps1 all clean; full-tree `git grep arch-gate` hits only docs/.mnfs history and superseded plans); the D-51 if-defeats-errexit pattern it carried dies with it
...
2026-07-28 · §6 (new paragraph) · ratified · BROWSER-DRIVE SEAM — THE HUB BORROWS THE FILE, IT DOES NOT RE-POINT THE STACK. A chip may not own the stack, so a screen criterion is discharged by the hub driving the CHIP's build. Re-pointing compose at the chip worktree fails: compose resolves each service's `env_file:` against the project directory, a worktree has no `.env`, and `--env-file` does not override a service-level `env_file:`. The workaround that suggests itself — creating `.env` in the chip's worktree — is FORBIDDEN, since it would place live provider credentials inside a chip session's reachable tree. (`-p <project>` remains required for any real re-point, since compose derives the project name from the directory and would otherwise fork a parallel stack on fresh volumes.) Working mechanism: hub does `git checkout <chip-branch> -- <granted files>` into its OWN checkout, drives, captures the rendered text, reverts with `git checkout HEAD -- <same files>`, verifying `git status --porcelain` both sides, and returns the text VERBATIM — drive is the hub's, reading stays the chip's. CHIP-ANCHORS-3 F-5 discharged this way at `b91c7507`
2026-07-28 · §11 "reading seats are given the DIFF" · amended · CARVE-OUT, INPUT ITEM 4: hub-MEASURED observables the seat cannot reproduce travel WITH the diff, as a committed gate artifact, whenever a criterion asserts a fact about running reality. Requested by CHIP-VINC-NEUTRO and the request is structural, not convenience: the chip cannot boot a server (§2 L2) and the seat has no browser, so withholding the measurement forces BOTH seats to infer reachability from the generator — the same source the chip inferred it from — and to reach the same wrong conclusion by construction. Not hypothetical: seven rounds, two independent gates, a false premise about a screen state survived every one, and one live drive killed it immediately (`04983aab`). The carve-out does not re-open the pack because it splits on a different axis than the original rule: item 4 is the hub MEASURING reality (a wire dump, a rendered screen, a query result — verbatim, contradictable by naming a row and a field), the exclusion is the chip ARGUING about it. Prose a seat can only agree or disagree with stays out; a capture it can refute is input. First artifact: `_hub-gate-vinc-neutro/DRIVE-EVIDENCE.md`, 9 live candidates, every rendered row derivable from the wire plus `QueueRow.tsx` on main
2026-07-28 · §11 (new subsection) · ratified · A DOMAIN ON WHICH THE TRANSFORM IS INJECTIVE DOES NOT MAKE THE FUNCTION INJECTIVE, WHEN THE FUNCTION HAS A FALLBACK BRANCH. The fallback is the IDENTITY, so it shares a codomain with the transform: every string the transform can produce is also a string the fallback can produce, and narrowing the domain does not separate the two — it MOVES inputs into the branch where the collision partner already lives. `providerDisplayName` restricted to `^[a-z0-9]+(_[a-z0-9]+)*$` closes `amazon-marketplace` (now verbatim) and leaves `amazon`/`Amazon` both painting `Amazon`, because `Amazon` fails the domain test and is therefore returned unchanged. Binds: an injectivity property is asserted on the CODOMAIN, never on one branch; a guard with an escape hatch is verified by asking what the hatch emits, not by verifying the guarded path. Corollary for gate ORDERS: R-24 binds the HUB's own orders — the round-8 order named a repair and asserted "this closes B-1 and R-1 together", which is a total claim about a program the hub had not run in its repaired form, and it was FALSE. Chip refuted it by execution before accepting it, which is the right order of operations and cost nothing; the hub confirmed by its own run and found the class WIDER than either side had it — case is one instance of two, `"Amazon Marketplace"` colliding with `"amazon_marketplace"` is the other, so the class is *an unmapped code whose literal string equals another code's typeset output*, not "case collapse". An order that says WHAT to repair is a dispatch; an order that also says what the repair CLOSES is a claim, and it is measured before it is sent or it is written as a question (CHIP-VINC-NEUTRO r9 field finding, ratified verbatim in mechanism, widened at the hub by execution — `HUB-VERIFY-round9.md`)

2026-07-28 · §11 (new subsection) · ratified by the operator · **R-24 IS SCOPED TO WHAT THE OPERATOR READS AND WHAT THE REPO PUBLISHES. A CODE COMMENT LEFT THE GATE.** R-24 ("a claim is TOTAL or it is a report") was ratified against pack prose, then extended into code (amendment above, 2026-07-28), and the extension metastasised: every docstring in the repo became a gate surface, the wave-2 criteria (`V9-1..V9-7`, `F8-1..F8-6`) were written almost entirely about the truth of prose, and the seats returned prose because that is what they were asked for. Cost: eight rounds on CHIP-ANCHORS-3 and nine on CHIP-VINC-NEUTRO, including one whole round on the injectivity of a function whose only live input is `ProviderCode: "mercado_livre"`, a hardcoded literal at `connectors/adapters/mercado_livre/capability_adapter.go:81`. R-24 now fires on exactly two surfaces: **a string the operator reads on screen**, and **a published contract** (OpenAPI, SDK types). A false code comment is a review note or a task, never a gate round. **The missing test is REACH, and it is measured, not argued: where does the value come from?** `provider_code`, `match_status`, `direction`, `side` and `confidence_band` are all Go constants flowing constant → column → API → FE, so there is no untrusted-input path; the round-9 prototype-chain finding (`providerDisplayNames["toString"]` returns a `function` under a declared `: string`) is real in mechanism and needs a developer to commit a strange literal before anything happens — the same trigger as R-1, which the same ruling had already filed as REPORT. Two weights, one bucket. Binds: (a) before BLOCKING on a false sentence, measure where the input originates — an input that can only come from a constant in this repo is REPORT; (b) before ordering a string repair, confirm the string RENDERS (`git grep` the field in `.tsx`) — a seat finding about an unrendered string is not an order; (c) count rounds against shipped behaviour, and when N rounds have produced only wording, the defect is in the GATE, not in the chip. Operator ruling of 2026-07-28 ("YAGNI … vamos limpar tudo que não precisa e finalizar a missão"), recorded with the surviving defects in `.mnfs/MIS-006-integracao-fundacao/CORTE-YAGNI.md`

2026-07-29 · §12 + §11 (new rule) · ratified by the operator · **A DESIGN DECISION NAMES THE PROBLEM IT SOLVES AND WHETHER THAT PROBLEM IS MEASURED OR IMAGINED. IMAGINED DOES NOT ENTER — NOT EVEN CHEAP.** Binds every hub ruling and every chip plan card: the decision states which problem it solves, and classifies it MEASURED (a number, an operator verbatim, a reproduced defect) or HYPOTHETICAL. Hypothetical is refused or parked as a report; "it's one line" is not a defense — the canonical artifact of the imagined problem is the `mercado_livre` display-name parse (a transform whose only live input is a hardcoded literal), which nobody ever decided to build: it accreted one reasonable line at a time and later cost nine gate rounds. Corollary, from the dispatch that motivated ratification: **a pack whose predicate depends on ERP data does not freeze before that data is measured.** CHIP-VENDAVEL shipped with acceptance number 3,822, measured with an incomplete predicate; the interview had covered USOPROD and CODEMP but nobody had asked the database about CODLOCAL, reservation, or the AD_ECOMMERCE value distribution — all three were measurable before dispatch, all three amended the rule mid-flight (final: 2,923), and only slice ordering luck kept the amendments in prose instead of rework. Business rules that read ERP fields are measured at interview time, not execution time. (Operator ruling 2026-07-29: "Sempre observar … YAGNI, para não termos problema igual àquela vez que estávamos fazendo parse de mercado_livre para Mercado Livre".)
