...
| **nova (review Fatia A)** | `TaxComponents.Unknown` e `TaxComponents.FCP` são **write-only em produção**: todas as ocorrências fora de `_test.go` são escritas dentro do próprio `icms.go` — nenhum leitor. (Esta linha citava sete `file:line`; a re-rodada do gate mostrou que **um** batia. Sem números aqui de proposito: o instrumento e o `grep` do criterio de aceite, que nao apodrece.) `decompose.go` monta a **sua própria** lista de desconhecidos a partir dos campos `*string` individuais e nunca lê `tax.Unknown`. Consequência medida: a Task A4 nomeou `"fcp"` em 8 caminhos e **esse nome não chega a lugar nenhum**; pior, `Decomposition` não tem campo FCP, então FCP-desconhecido é invisível na tela. Duas listas paralelas onde só uma é lida = divergência silenciosa garantida. Aceite não é teste verde: é um leitor de produção de `.Unknown` (ou a remoção do campo), verificável por `grep '\.Unknown' --include='*.go' \| grep -v _test.go` retornando ≥1 leitura fora de `icms.go` |
