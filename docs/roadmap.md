...
- R1 challenge found three valid proof/contract defects now corrected in the candidate: source candidate identity is structurally unique by opaque candidate key; each requirement value-spec family is mechanically coupled to the matching source-value family while `not_applicable` remains override-only; and semantic proof runs against the parsed Redocly bundle rather than raw YAML substrings.
