...
**Provenance.** Transcribed by the HUB from the chip's BLOCKED event body, filed per R-23. The chip
holds `_chip-anchors/verdict-r6-recheck.md`, written at a different time by a different party from
the same source. Two holders make a later discrepancy detectable.

**Honest limit** (R-23): two copies do NOT make either verifiable against the vanished original.
What they remove is unilateral control over the text that judges the transcriber.

**Correction, R-26.** The first version of this file was labelled VERBATIM and was not. I had
collapsed the reviewer's markdown links to absolute worktree paths into short backticked names, and
replaced its curly quotes with straight ones. Both edits were mine, neither changed meaning, and the
label was false regardless — VERBATIM is a claim about form. The discrepancy was found by comparing
this file against the chip's copy, which is the check R-23 was created to make possible; it fired on
its first real use, against the party who wrote the rule. The body below now sits inside a fence so
that faithful reproduction is possible, and every word of mine is above it.

- Corrective tip re-checked: `56598bea` · Transcription tip: `60bcde9b` · Frozen code tip: `85b6c367`
