...
| M05-C5 | Listing sem EAN e sem SKU utilizável fica sem candidato, com motivo visível, e nada é aprovado | candidato sem âncora → `status=NO_CANDIDATE` + campo de motivo não-vazio (honesto, não silencioso), zero aprovação. **Emenda do hub 2026-07-25 (D-121-3):** a redação original pedia `REVIEW`; `/vinculos` chaveia a linha honesta "sem candidato / Criar produto / Ignorar" **e o guard do batch-select** exatamente em `NO_CANDIDATE` (`QueueRow.tsx:77`, `VinculoDrawer.tsx:63`), então emitir `REVIEW` transformaria um anúncio sem âncora numa linha de revisão comum, aprovável em massa para o vínculo errado — a classe de regressão invisível do `/catalogo`-503. O estado correto é `NO_CANDIDATE`; a intenção do critério (nada silencioso, nada automático) é a mesma | L1 ran | F-02 |
