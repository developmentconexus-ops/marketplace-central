...
| M05-C2 | Candidato EAN-único-sem-SKU auto-aprova | `SELECT status FROM product_links WHERE ... = 90008` retorna aprovado, sem nenhuma chamada a `ApproveCandidate`/endpoint manual na sessão de teste | L2 ran | F-02 |
| M05-C3 | Auto-aprovação grava linha E10 correta | `SELECT rule_matched, actor, collisions_at_decision FROM <tabela_audit_E10> WHERE link_id = ...` retorna `actor=system`, `collisions_at_decision=1` e `rule_matched` = a âncora que de fato decidiu (`exact_ean_unique` no par de C2) | L2 ran | F-03 |
...
| M05-C14 | CODPROD-único auto-aprova mesmo SEM EAN | anúncio com `seller_sku` = CODPROD válido e único, sem EAN → aprovado + E10 `rule_matched=exact_codprod_unique, actor=system` | L2 ran | F-02 |
| M05-C15 | CODPROD + EAN concordantes auto-aprovam com regra própria | as duas âncoras apontando o mesmo produto → aprovado + E10 `rule_matched=concordant_codprod_ean` | L2 ran | F-02 |
...
- **MC-05** (`validation-contract.md` da missão) — coberto por M05-C1..C6 e C14..C20.
  IO A: produto 90008 EAN único → candidato existe E `product_links` aprovado + E10
  `actor=system, rule=exact_ean_unique` (M05-C1/C2/C3). As outras duas regras de auto-approve
  ratificadas em D-121 → M05-C14 (CODPROD-único) e M05-C15 (concordante). Os três caminhos de
  REVIEW → M05-C4 (colisão), M05-C16 (conflito CODPROD≠EAN), M05-C17 (hard-negative). A âncora
  de SKU corrigida → M05-C18/C19.
...
M-05 fecha `validated` só com todos os M05-C1..C20 em `ran` (ou `could-not-run` nomeado com
...
|----|----------|----------------------------|
| M05-U1 | Pós-import real: vínculos de âncora não-ambígua aparecem AUTO-APROVADOS na tela de vínculos como o usuário vê; contadores da tela batem com o DB | browser drive + SELECT de conferência |
...
hub re-dirige U1/U3 no dev stack depois que o operador completar o OAuth. M05-U2 é executável
contra fixture na tela de vínculos.
