...
BASE-SHA `e3c081ae43b72af070185939253b745080acf68b`. Chip tip `5807d634`.

Commits (`git diff e3c081ae..5807d634`, 33 files, zero under `apps/web`):
...
- `84323d0a` F-01 — internal generation trigger on the xlsx import and on the products sync, failure logged and isolated
- `5807d634` correctives — the four defects the round-1 dual gate found (see **Round-1 dual gate** below)
...
| L1 product_links | `go test -count=1 ./internal/modules/product_links/... -v` | **86 tests PASS** (69 before the correctives), zero FAIL |
...
| L2 integration (hermetic PG) | `pwsh -NoProfile -File scripts/harness.ps1 -Command integration` | **status=passed**, `migrations_first=69`, `migrations_second=0`, `run_id=4f753adba75e4afdb37b82031792bc25`, `run_dir=scripts/.runs/4f753adba75e4afdb37b82031792bc25` |
...
nullable, no default, honest-unknown), AC-04 (zero `validEANCounts`/`identityQuality` in
...
**Round 2 of the dual gate runs against `5807d634`** — per the D-120 lesson, correctives get a
real second cold pass and a real second refuter, never a self-verified round 2.
