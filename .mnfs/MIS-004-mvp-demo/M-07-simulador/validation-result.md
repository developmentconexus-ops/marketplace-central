...
1. 📋 **F-difal-1** — hub ruling on per-simulation DIFAL (option A accept global-only + warning for demo [recommended] / option B contract-extension slice). ESCALATION sent to HUB v2.
2. ⏳ **P7 live-drive** — §tela + §mutacao evidence on the hub dev-stack (REQUEST dev-stack pending). C05/C06 are live-drive criteria.
