...
Three small read APIs. **Dashboard summary** `GET /dashboard/summary?installation_id=`: composes existing module reads — listings summary (IC-02), pendências counts (sync errors, unresolved links, below_margin, sem GTIN), orders today/7d counts, last sync times per module — each counter nullable; a failed sub-read yields null for that counter plus a `degraded: [source]` list, never a 500 or a 0. **Orders read** `GET /orders` (cursor, filters: status, date range, q; `filter.fulfillment` → 400 `unsupported_filter` this mission — hub ruling 2026-07-16, no canonical fulfillment source in storage) + `GET /orders/{orderId}` over existing orders tables — canonical fields only, provider raw behind adapter; buyer/currency/fulfillment/NF-state honest-null (ADR-17) until a successor adds adapter-ingestion columns; canonical order timestamp for date filters/counters = `provider_created_at`, null never defaulted. **Sync runs** `GET /sync/runs` (cursor, filters: module, status; sort `started_at DESC`, newest first) over `integration_operation_runs`, window fixed at last 90 days (`started_at >= now() - 90d`; older runs excluded from the listing — mission §Audit & history "sync central 90d view"). OpenAPI + sdk-runtime same commit. This feature owns the new `/dashboard` and `/sync` server prefixes; their Vite dev-proxy rows are FE-seam work (mission plan zero-frontend rule overrides — hub 2026-07-16): CHIP-M02 (W1, per mission Clarified Decisions "dev proxy additions land in M-02") or the W3 FE chip adds them.
...
- While orders exist, when listed with `filter.status=`, the API shall return canonical order rows (id, provider_order_id, buyer nickname nullable, total (payments sum, nullable), currency nullable, status, fulfillment nullable, NF state nullable, created_at = `provider_created_at` nullable) cursor-paginated newest first — nullable fields honest-null per ADR-17 (hub ruling 2026-07-16).
