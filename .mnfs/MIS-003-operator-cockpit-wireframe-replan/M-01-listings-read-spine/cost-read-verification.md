...
ESCALATION sent to hub `local_efa46c30` with 3 research asks: (1) is per-(GRUPOICMS, UF-dest) worst-case output-ICMS+DIFAL deterministically derivable from TGFICM with as-of discipline? (2) worst-case single rate vs full per-state matrix (latter expands read contract with a per-UF dimension)? (3) M-01 read-spine scope vs mission-level defer (below_margin is 1st-class IC-02 exception D-17 → dropping risks gate fail; simulating = net-new scope). Awaiting hub ruling → D-22. below_margin (F-02 Slice 2/5) PARKED; all other F-02 slices + F-01 proceed.
