...
| M03-U2 | /pedidos lista e summary sem regressão visível pós-switch. **Unidade nomeada (emenda do hub 2026-08-01, o critério antes não dizia contagem DE QUÊ):** o before/after compara os **4 KPIs de bucket do summary de /pedidos** (contagem de PEDIDOS por bucket, não de itens nem de linhas da lista), tenant e janela idênticos nas duas capturas. Tolerância = **ZERO diferença não explicada**: toda variação tem que ser atribuída, pedido a pedido, ao braço que hoje passa `shipmentStatus=""` (`order_repo.go:378`) e agora recebe o status real — se um pedido muda de bucket, o evidence pack nomeia o pedido, o bucket velho, o novo e o shipment status que causou a troca. Diferença sem essa atribuição = blocking. | browser drive lista + summary; JSON dos 4 KPIs before/after salvo + tabela de atribuição por pedido |
